...
WrenchStampedDisplay::WrenchStampedDisplay()
{
  ...
  torque_scale_property_ = new rviz::FloatProperty("Torque Arrow Scale", 2.0, "torque arrow scale", this,
                                                   SLOT(updateProperties()));
...
void WrenchStampedDisplay::processMessage(const geometry_msgs::WrenchStamped::ConstPtr& msg)
{
  ...
  {
    ROS_ERROR_THROTTLE(
        1.0, "Wrench position contains NaNs. Skipping render as long as the position is invalid");
    return;
  }
